job('MNTLAB-skuzmich-main-build-job') {
  blockOnDownstreamProjects()

  parameters {
      choiceParam('BRANCH_NAME', ['skuzmich (default)', 'master'])
      ...
      activeChoiceParam('BUILDS_TRIGGER') {
            ...
            groovyScript {
             ...
             def list=[]
	         (1..4).each {
	         list.add("MNTLAB-skuzmich-child${it}-build-job")
       }
	      return list
          ''')
        }
...
for(i in 1..4) {
  job("MNTLAB-skuzmich-child${i}-build-job") {
    parameters {
